...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		if (colonIndex < 0)
			throw new TreeException("colon index out of bounds");
		...
		if (root == null) {
			...
			if (parentNode == null) {
				throw new TreeException("apparent parent is null :(");
			}
		}
		
		// Add child nodes to parentNode.
		for (String s: childrenArray) {
			TreeNode newKid = new TreeNode(s);
			parentNode.addChild(newKid);
		}
	}
	...
	TreeNode getMostRecentCommonAncestor(String name1, String name2) throws TreeException
	{
		...
		if (node1 == null)
			throw new TreeException("thERe's nO rOoT");
		TreeNode node2 = root.getNodeWithName(name2);		// node whose name is name2
		if (node2 == null)
			throw new TreeException("thERe's nO rOoT 2");
